function [x_hat, mask_est, obj, times, err_x, err_mask] = blindInpaint(varargin)
...
obj(1) = norm(g(:)-u(:)-v(:))^2 + tau1*TVnorm(u) + tau2*sum(v(:)~=0);
...
for iter = 1:maxiters
    ...
    obj(iter+1) = norm(g(:)-u(:)-v(:))^2 + tau1*TVnorm(u) + tau2*sum(v(:)~=0);

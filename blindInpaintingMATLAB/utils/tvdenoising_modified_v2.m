function [xe, times, mses, obj]=tvdenoising_modified_v2(y,lambda,niter, x_true, stopCriterion, tol)
% Usage:
% function u=tvdenoising(y,lambda,niter)
%
% Chambolle projection's algorithm:
%
% @article{chabolle_TV:JMIV,
...
%  issn = {0924-9907},
%  pages = {89--97},
%  doi = {http://dx.doi.org/10.1023/B:JMIV.0000011321.19549.88},
%  publisher = {Kluwer Academic Publishers},
%  address = {Norwell, MA, USA}, (1)
%
...
xe = y;
mses(1) = norm(xe(:)-x_true(:))^2/numel(xe);

obj(1) = 0.5*norm(y(:)-xe(:))^2 + lambda*TVnorm(xe);
...
for i=1:niter
    ...
    mses(i+1) = norm(xe(:)-x_true(:))^2/numel(xe);
    obj(i+1) = 0.5*norm(y(:)-xe(:))^2 + lambda*TVnorm(xe);
    times(i+1) = cputime-t0;
    
    
    if stopCriterion > 0
        if stopCriterion == 1
            criterion = norm(xe(:)-xe_prev(:))/norm(xe(:));
        else
            if stopCriterion == 2
                criterion = abs(obj(i+1)-obj(i))/obj(i+1);
            else
                error('Invalid stopping criterion.\n')
            end
        end
        if criterion < tol
            fprintf('Stopping criterion satisfied.\n')
            return;
        end
    end
